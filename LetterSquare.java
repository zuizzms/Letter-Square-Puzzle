...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {        
        if (charNum == 0) {
            if (wordNum == 0) 
                return true;
            else if (letter.equals(lastLetter(words[wordNum-1])))
                return true;
            else
                return false;
        }
        else {
            String newWord = words[wordNum] + letter;
            if (onSameSide(letter, lastLetter(words[wordNum])))
                return false;
            else {
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        if (allLettersUsed() && dictionary.hasFullWord(this.words[wordNum])) {
            ...
            for (int i = 0; i < letters.length; i++) {
                if (isValid(letters[i], wordNum, charNum)) {
                    ...
                    if (solveRB(wordNum, charNum+1, maxWords))
                        ...
                        if (dictionary.hasFullWord(this.words[wordNum]) 
                            && this.words[wordNum].length() >= 3
                            && solveRB(wordNum+1, 0, maxWords))
                            return true;
                        else
